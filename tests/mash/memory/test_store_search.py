...
class SQLiteStoreKeywordSearchTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
    ...
    async def _insert_turn_with_app(
        self,
        *,
        turn_id: str,
        session_id: str,
    ...
    async def test_keyword_search_returns_empty_for_non_positive_limit_or_blank_query(
        self,
    ) -> None:
    ...
    async def test_keyword_search_is_column_scoped_and_token_and(self) -> None:
    ...
    async def test_keyword_search_filters_by_session_id(self) -> None:
        wanted = await self._save_turn(
    ...
    async def test_keyword_search_filters_by_app_id(self) -> None:
        await self._insert_turn_with_app(
            turn_id="app-a-turn",
            session_id="s1",
            app_id="app-a",
    ...
    async def test_keyword_search_filters_by_app_id_for_turns_saved_via_save_turn(
        self,
    ) -> None:
        wanted = await self._save_turn(
            session_id="s1",
    ...
    async def test_keyword_search_combines_session_and_app_filters(self) -> None:
        await self._insert_turn_with_app(
            turn_id="match",
            session_id="s1",
            app_id="app-a",
