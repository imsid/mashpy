...
class _BaseDefinition(MashRuntimeDefinition):
    ...
    def build_store(self):
        return SQLiteStore(str(self.root / "state.db"))
...
class MashAgentServerTests(unittest.TestCase):
    ...
    def test_boots_and_shutdown_without_mcp(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            runtime = MashAgentServer.from_definition(definition)
    ...
    def test_app_id_mismatch_raises(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaises(ValueError):
                MashAgentServer.from_definition(_MismatchedDefinition(Path(tmp)))

    def test_process_user_message_saves_turn_and_tokens(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            runtime = MashAgentServer.from_definition(_BaseDefinition(Path(tmp)))
            response = Response(
                text="hello",
                context=Context(),
